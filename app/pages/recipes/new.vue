...
const nutritionBasis = ref<'per_serving' | 'per_100g'>('per_100g')
...
async function submit() {
  ...
  try {
    ...
    error.value = err.data?.message || err.message || 'Не удалось сохранить рецепт'
  } finally {
    saving.value = false
  }
}
  ...
  <div class="px-4 pt-safe pb-6">
    ...
    <form class="mt-4 space-y-5" @submit.prevent="submit">
      ...
      <button type="submit" class="btn-primary w-full" :disabled="saving">
        ...
        {{ saving ? 'Сохраняю…' : 'Сохранить рецепт' }}
      </button>
    </form>
  </div>
</template>
